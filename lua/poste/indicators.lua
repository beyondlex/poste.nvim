...
function M.set_indicator(buf, line_0, status, latency_ms, assertion_results)
  ...
  elseif status == "success" then
    ...
    local success_virt = build_virt_text("✓", "PosteSuccess", latency_ms, assertion_results)
    if #success_virt > 0 then
  ...
  elseif status == "error" then
    clear_all_virt_text(buf)
    local error_virt = build_virt_text("✘", "PosteError", latency_ms, assertion_results)
    if #error_virt > 0 then
      vim.api.nvim_buf_set_extmark(buf, indicator_ns, line_0, 0, {
